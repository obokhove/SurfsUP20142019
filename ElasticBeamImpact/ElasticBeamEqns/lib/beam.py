...
#        self.phi_vect = fd.Function(self.V, name="Surface forcing")
    ...
    def initial_conditions(self):
        ...
        ic2 = fd.project( fd.Expression(["0.1*(1-cos(pi*x[2]/Lz/2.))",0.,0.], Lz=self.Lz), self.V )
...
#        self.read_raw()
    ...
    def surface_force(self):
        ...
        class MyExpression(fd.Expression):
            def eval(self, value, X):
                if X[2]>d:
                    value[:] = [A*fd.sin((X[2]-d)/l * fd.pi/2.)**2,0.,0.]
                else:
                    value[:] = [0.,0.,0.]
        ...
        T.interpolate(MyExpression()) # surface force / area
        return T   
...
#        f = fd.Constant((0, 0, -self.g)) # body force / rho
        ...
        # Compute solution
        self.DBC = fd.DirichletBC( self.V, fd.Expression([0.,0.,0.]), self.bottom_id)
