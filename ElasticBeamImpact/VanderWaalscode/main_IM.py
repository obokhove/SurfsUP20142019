...
Mixed space, modified midpoint scheme.
"""
...
phi_m = (phi_next+phi)/2.
rho_m = (rho_next+rho)/2.
...
# equations
F_phi = ( (phi_next - phi)/dt + 1./2.*inner(grad(phi_m), grad(phi_m)) + Q(rho_m) ) * var_rho * dx
F_rho = ( var_phi*(rho_next-rho)/dt - rho_m*inner( grad(var_phi), grad(phi_m) ) ) * dx
F = F_phi + F_rho
