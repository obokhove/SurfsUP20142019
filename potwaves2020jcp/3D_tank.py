...
if input_data=="measurements":#------------- Interpolate measurements -------------#
    #------------------------------- Wavemaker motion -------------------------------#
    # 04-2020: needs changing
    # f = Function(V)
    # x, y = SpatialCoordinate(hor_mesh)
    # wm2=wm_data[1], wm1=wm_data[0], t1=t_data[0], t2=t_data[1]
    # WM_expr = f.interpolate(((wm2*(t-t1) - wm1*(t-t2))/(t2-t1))*0.5*(1.0+copysign(1.0,Lw-x[0]))
    WM_expr = \
    ...
    # dWM_dt_expr = f.interpolate(((dwm2*(t-t1) - dwm1*(t-t2))/(t2-t1))*0.5*(1.0+copysign(1.0,Lw-x[0])))
    dWM_dt_expr = \
...
t_aux = t
update_wm = 'Yes' ;
smallfac = 10.0**(-10.0)  #  Onno 13-04-2020
while t<=Tend+smallfac: #  while t<Tend-dt: Onno 15-04-2020
    """ *****************************************************************************
        *                               SAVE FUNCTIONS                              *
        ***************************************************************************** """
    if t_save-smallfac < t: # Onno 13-04-2020WTF;have showed how to do not this:t_save <= t: ???
        # print('Progress: ', 1.0*t, ' %') # 100*t/Tend, ' %') Onno 13-04-2020 WTF???
        PETSc.Sys.Print('Progress: ', 1.0*t, ' %')
        #-------------------------------------------------------------------------------#
        #                                    ENERGY                                     #
        #-------------------------------------------------------------------------------#
