...
"""
    ...
    ***********************************************************************************"""
#---------------------------- Topography ----------------------------#
beach_expr =0.5*(1.0+1.0*sign(x[0]-xb))*sb*(x[0]-xb)*(1.0 + 0.05*cos(1.5*x[0]*x[1]))
H_expr= H0-beach_expr # Depth at rest H(x)
H_intp = Interpolator(H_expr, H)
H_intp.interpolate()
beach.interpolate(H0-H)                                   # Beach b(x,y)
beach_file = File("beach.pvd")
beach_file.write(beach)
#----------------------------------------------------------------------------------------#
#                                       Wavemaker                                        #
#----------------------------------------------------------------------------------------#
if input_data=="measurements":#------------- Interpolate measurements -------------#
    #------------------------------- Wavemaker motion -------------------------------#
    WM_expr = \
    Expression("((wm2*(t-t1) - wm1*(t-t2))/(t2-t1))*0.5*(1.0+copysign(1.0,Lw-x))",\
               wm2=wm_data[1], wm1=wm_data[0], t1=t_data[0], t2=t_data[1], t=t, Lw=Lw)
    WM.interpolate(WM_expr)

    #------------------------------- Wavemaker velocity -----------------------------#
    dWM_dt_expr = \
    Expression("((dwm2*(t-t1) - dwm1*(t-t2))/(t2-t1))*0.5*(1.0+copysign(1.0,Lw-x))",\
    ...
    if dim == "2D":
        L = sign(x[0])
        WM_expr = L #-0.5*(1+abs(Lw-x[0])/(Lw-x[0]))*gamma*cos(w*t_expr)
    elif dim == "3D":
        WM_expr=  -0.5*(1+1.0*sign(Lw-x[0]))*gamma*cos(w*t_expr)#*(x[1]-0.5*Ly)/(0.5*Ly)
        dWM_dt_expr = 0.5*(1+1.0*sign(Lw-x[0]))*gamma*w*sin(w*t_expr)#*(x[1]-0.5*Ly)/(0.5*Ly)
        dWM_dy_expr = 0.0*x[0]#-0.5*(1+1.0*sign(Lw-x[0]))*gamma*cos(w*t_expr)/(0.5*Ly)

    WM_intp = Interpolator(WM_expr,WM)
    dWM_dt_intp = Interpolator(dWM_dt_expr,dWM_dt)
    dWM_dy_intp = Interpolator(dWM_dy_expr,dWM_dy)

    WM_intp.interpolate()
    dWM_dt_intp.interpolate()
    dWM_dy_intp.interpolate()
if scheme=="SV":
    WM_half_intp = Interpolator(WM_expr,WM_half)
    dWM_dt_half_intp = Interpolator(dWM_dt_expr,dWM_half_dt)
    dWM_dy_half_intp = Interpolator(dWM_dy_expr,dWM_half_dy)
    WM_half_intp.interpolate()
    dWM_dt_half_intp.interpolate()
    dWM_dy_half_intp.interpolate()
WM_n1_intp = Interpolator(WM_expr,WM_n1)
dWM_n1_dt_intp = Interpolator(dWM_dt_expr,dWM_n1_dt)
dWM_n1_dy_intp = Interpolator(dWM_dy_expr,dWM_n1_dy)
WM_n1_intp.interpolate()
dWM_n1_dt_intp.interpolate()
dWM_n1_dy_intp.interpolate()
...
for i in range(0,Nz):
    for j in range(0,Nz):
        it+=1
        print('... filling '+str(it)+'/'+str(Nz*Nz))
...
if scheme=="SE": #_____________________________________________ Symplectic-Euler ______________________________________________#
    ...
    WF_h_psi = DW_solvers.WF_h_SE(dim, n_z, g, beach, H0, Lw, WM, dWM_dy, dWM_dt, dt, delta_psi, delta_hat_star, h_n0, h_n1, x_coord, psi_1_n0, hat_psi_star, A11, AN1, ANN, B11, B1N, BN1, BNN, C11, CN1, CNN, D11, D1N, DN1, DNN, M11, M1N,MN1, MNN, S11, SN1, SNN, I1, IN)

    #----------------------------------------- Step 2 : Update psi_1 at time t^{n+1}: -----------------------------------------#
    A_psi_s, L_psi_s = DW_solvers.WF_psi_SE(dim, g, H, beach, H0, Lw, WM, WM_n1, dWM_dy, dWM_dt, dt, x_coord, delta_h, psi_1, psi_1_n0, hat_psi_star, h_n1, A11, AN1, ANN, B11, B1N, BN1, BNN, C11, CN1, CNN, D11, D1N, DN1, DNN, M11, MN1, MNN, S11, SN1, SNN, I1, IN)

    #----------------------------------------- Step 3 : Update psi_i at time t^{n+1}: -----------------------------------------#
    A_hat, L_hat = DW_solvers.WF_hat_psi_SE(dim, beach, H, H0, g, n_z, Lw, x_coord, WM, dWM_dt, dWM_dy, dt, delta_hat_psi, hat_psi, h_n0, psi_1_n0, A11, AN1, ANN, B11, B1N, BN1, BNN, C11, C1N, CN1, CNN, D11, D1N, DN1, DNN, M11, MN1, MNN, S11, SN1, SNN, I1, IN)
    ...
    WF_psi_star = DW_solvers.WF_psi_half_SV(beach, dim, n_z, g, H, H0, Lw, x_coord, WM, WM_half, dWM_dy, dWM_dt, dWM_half_dy, dWM_half_dt, dt, delta_psi, delta_hat_star, psi_1_n0, psi_1_half, hat_psi_star, h_n0, M11, M1N, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN, I1, IN, B11, B1N, BN1, BNN, C11, C1N, CN1, CNN )
    #----------------------------- Step 2 : Update h^{n+1} and psi_i at time t^** simulataneously: ----------------------------#
    WF_h_psi = DW_solvers.WF_h_SV(beach, dim, n_z, Lw, H0, g, dt, x_coord, WM, WM_half, dWM_half_dy, dWM_half_dt, delta_psi, delta_hat_star, h_n0, h_n1, psi_1_half, hat_psi_star, hat_psi_aux, M11, M1N, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN, I1, IN, B11, B1N, BN1, BNN, C11, C1N, CN1, CNN )

    #----------------------------------------- Step 3 : Update psi_1 at time t^{n+1}: -----------------------------------------#
    a_psi_1, L_psi_1 = DW_solvers.WF_psi_n1_SV(beach, dim, H0, H, g, x_coord, delta_h, Lw, WM_n1, WM_half, dt, psi_1_half, psi_1, dWM_half_dt, dWM_half_dy, hat_psi_aux, h_n1, M11, M1N, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN, I1, IN, B11, B1N, BN1, BNN, C11, C1N, CN1, CNN  )

    #----------------------------------------- Step 4 : Update psi_i at time t^{n+1}: -----------------------------------------#
    A_hat, L_hat = DW_solvers.WF_hat_psi_SV(beach, dim, n_z, Lw, H0, H, WM, x_coord, dt, dWM_dt, dWM_dy, delta_hat_psi, hat_psi, h_n0, psi_1_n0, M11, M1N, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN, I1, IN, B11, B1N, BN1, BNN, C11, C1N, CN1, CNN  )
...
"""
    ...
    *                        Saving Files                       *
    ************************************************************* """
save_waves, save_WM, WM_file, Energy_file, T_file, README_file = saving_files(save_path)
...
if dim=='2D': # Extend the 1D horizontal mesh (x) to 2D horizontal mesh (x,y)
    ...
    beach_s_2D = Function(V_2D).interpolate(Expression("0.5*(1+copysign(1.0,x-xb))*slope*(x-xb)",
...
xx,yy,zz = SpatialCoordinate(mesh_3D)
Test_x_Lw.interpolate(0.5*(1.0+1.0*sign(Lw-xx)))
...
while t<Tend-dt:
    ...
    if t_save <= t:
        ...
        #                                    ENERGY                                     #
        #-------------------------------------------------------------------------------#
        save_energy(beach, h_n0, psi_1_n0, hat_psi_n0, WM, dWM_dy, dWM_dt, H, x_coord, Lw,
                    H0, g, A11, AN1, A1N, ANN, B11, BN1, B1N, BNN, C11, CN1, C1N, CNN, M11, M1N, MN1, MNN, D11, D1N, DN1,
                    DNN, S11, S1N, SN1, SNN, I1, IN, Energy_file, t, dim, T_file)
