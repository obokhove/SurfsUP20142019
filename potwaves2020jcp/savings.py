...
def saving_files(save_path):
    ...
    T_file = open(os.path.join(save_path, 'Time.txt'), 'w')
    README_file = open(os.path.join(save_path, 'README.txt'), 'w')
    return save_waves, save_WM, WM_file, Energy_file,T_file, README_file
...
def save_energy(b, h_n0, psi_1_n0, hat_psi_n0, WM, dWM_dy, dWM_dt, H, x_coord, Lw, H0, g, A11, AN1, A1N, ANN, B11, BN1, B1N, BNN, C11, CN1, C1N, CNN, M11, M1N, MN1, MNN, D11, D1N, DN1, DNN, S11, S1N, SN1, SNN, I1, IN, Energy_file, t, dim, T_file):
    if dim =="3D":
        X =x_coord-Lw
        U = (X*dWM_dy)
        V = Lw**2 + U**2
        W = Lw-WM
        energy = assemble((0.5*h_n0*(M11*op(V,W,U,psi_1_n0,psi_1_n0) + 2*dot(MN1,op(V,W,U,psi_1_n0,hat_psi_n0))
                                      +(V/W)*dot(hat_psi_n0.dx(0),dot(MNN,hat_psi_n0.dx(0)))\
                                      +W*dot(hat_psi_n0.dx(1),dot(MNN,hat_psi_n0.dx(1)))\
                                      +2*U*dot(hat_psi_n0.dx(0),dot(MNN,hat_psi_n0.dx(1))))\
                           -(op(V,W,U,h_n0,psi_1_n0)*(D11*psi_1_n0 + dot(D1N,hat_psi_n0))) \
                           -dot(op(V,W,U,h_n0,hat_psi_n0), (DN1*psi_1_n0 +dot(DNN, hat_psi_n0)))\
                           -H0*(op(V,W,U,b,psi_1_n0)*(B11*psi_1_n0 + dot(B1N,hat_psi_n0))) \
                           -H0*dot(op(V,W,U,b,hat_psi_n0), (BN1*psi_1_n0 +dot(BNN, hat_psi_n0)))\
                           +(0.5/h_n0)*op(V,W,U,h_n0,h_n0)*(S11*psi_1_n0**2  + dot(hat_psi_n0, 2.0*SN1*psi_1_n0 \
                                                                                          + dot(SNN,hat_psi_n0)))\
                           +(H0/h_n0)*op(V,W,U,b,h_n0)*(C11*psi_1_n0**2  + dot(hat_psi_n0, 2.0*CN1*psi_1_n0 + dot(CNN,hat_psi_n0)))\
                           +0.5*(H0*H0/h_n0)*(W+op(V,W,U,b,b))*(A11*psi_1_n0**2 + dot(hat_psi_n0, (2.0*AN1*psi_1_n0  + dot(ANN,hat_psi_n0))))\
                           +0.5*H0*g*W*(h_n0-H)**2)*dx)
#    elif dim=="2D":
#        energy = assemble(( 0.5*(h_n0/W)*(Lw**2)*((psi_1_n0.dx(0)**2)*M11 \
#                                                        + dot(hat_psi_n0.dx(0), \
#                                                              (2.0*MN1*psi_1_n0.dx(0) \
#                                                               + dot(MNN,hat_psi_n0.dx(0))))) \
#                           -((1/W)*(Lw**2)*h_n0.dx(0))*(psi_1_n0.dx(0)*(D11*psi_1_n0 + dot(D1N,hat_psi_n0)) \
#                                                              +dot(hat_psi_n0.dx(0), \
#                                                                   (DN1*psi_1_n0+dot(DNN, hat_psi_n0))))\
#                           +(1/h_n0)*((0.5/W)*(h_n0.dx(0)**2)*(Lw**2))*(S11*psi_1_n0**2 \
#                                                                              + dot(hat_psi_n0, 2.0*SN1*psi_1_n0 \
#                                                                                    + dot(SNN,hat_psi_n0)))\
#                           +0.5*(W*H0*H0/h_n0)*(A11*psi_1_n0**2 + dot(hat_psi_n0, \
